...
public interface StudentInfoRepo extends JpaRepository<Student, String> {
	...
	@Query(value="select regno,rollno,age,name,deptcode,deptname,academic_year,passedout_year,attendance_status,date_of_record from student where lower(name) like lower(%:name%)",nativeQuery = true)
